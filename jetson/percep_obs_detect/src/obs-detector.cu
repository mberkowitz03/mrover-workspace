...
pair<ObsDetector::Tag, ObsDetector::Tag> ObsDetector::findARTags(cv::Mat &src, cv::Mat &depth_src, cv::Mat &rgb) {  //detects AR tags in source Mat and outputs Tag objects for use in LCM
    // RETURN:
    ...
    pair<ObsDetector::Tag, ObsDetector::Tag> discoveredTags;
    if (ids.size() == 0) {
...
//check camera.cpp and camera.hpp in main to see if this is necessary
